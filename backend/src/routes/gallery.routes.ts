...
router.post('/', requireAdmin, upload.single('image'), compressUpload, addGalleryItem);
// '/order' must stay above '/:id' so it isn't swallowed by the param route
router.put('/order', requireAdmin, updateGalleryOrder);
router.put('/:id', requireAdmin, upload.single('image'), compressUpload, updateGalleryItem);
